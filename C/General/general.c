...
{
	linkedList* l = (linkedList*) ncalloc(1, sizeof(linkedList));
   l->size = 0;
   l->start = (LNode*) ncalloc(1, sizeof(LNode));
   l->end = l->start;
	return l;
}
...
{
	LNode* p = l->start->next;
	while (p) {
      switch (sizeof(p->data))
      {
      case 1:
         printf("%c ", p->data);
         break;
...
void addToEnd_list_linked(linkedList* l, listType data) {
   if (l) {
      LNode* d = ncalloc(1, sizeof(LNode));
      d->data = data;
      l->end->next = d;
      l->end = d;
      l->size += 1;
